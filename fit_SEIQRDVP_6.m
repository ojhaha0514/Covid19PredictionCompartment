function [mu1,kappa1,alpha1,gamma1,omega1] = fit_SEIQRDVP_6(C,R,D,Vaccination1,Vaccination2,Vaccination3,Vaccination4,...
    Npop,E0,I0,V10,V20,V30,V40,P0,time,guess,pIndex,start_delta,fit,varargin)
%% Inputparseer
p = inputParser();
...
Vacc4 = repelem(Vaccination4/24, 24);
...
%% nested functions
    ...
    function [output] = SEIQRDVP_for_fitting(para,t0)
        ...
        %% Initial conditions
        N = numel(t);
        Y = zeros(11,N); %  There are seven different states
...
%             e_var = 0.1*(1-p_var)+(0.1+0.05)*p_var;
            ...
            s1 = -(1+p_var*0.97)*(mu)*s0*i0/Npop-Vacc1(ii);
            e1 = (1+p_var*0.97)*(mu)*(s0+(e_var1*v10+e_var2*v20+e_var3*v30))*i0/Npop - kappa*e0;
            i1 = kappa*e0 - alpha*i0;
            q1 = alpha*i0 - gamma*q0;
            r1 = (1-f)*gamma*q0;
            d1 = f*gamma*q0;
            v11 = Vacc1(ii)-Vacc2(ii) - (1+p_var*0.97)*(mu)*e_var1*v10*i0/Npop;
            v21 = Vacc2(ii)-Vacc3(ii) - (1+p_var*0.97)*(mu)*e_var2*v20*i0/Npop;
            v31 = Vacc3(ii) - (1+p_var*0.97)*(mu)*e_var3*v30*i0/Npop - omega*v30;
            p1 = omega*v30;
...
%             
            ...
            s2 = -(1+p_var*0.97)*(mu)*(s0+dt*s1/2)*(i0+dt*i1/2)/Npop-Vacc1(ii);
            e2 = (1+p_var*0.97)*(mu)*(s0+(e_var1*v10+e_var2*v20+e_var3*v30)+...
            ...
            v12 = Vacc1(ii) - Vacc2(ii) - (1+p_var*0.97)*(mu)*e_var1*(v10+dt*v11/2)*(i0+dt*i1/2)/Npop;
            v22 = Vacc2(ii) - Vacc3(ii) - (1+p_var*0.97)*(mu)*e_var2*(v20+dt*v21/2)*(i0+dt*i1/2)/Npop;
            v32 = Vacc3(ii) - (1+p_var*0.97)*(mu)*e_var3*(v30+dt*v31/2)*(i0+dt*i1/2)/Npop - omega*(v30+dt*v31/2);
            p2 = omega*(v30+dt*v31/2);
            
            s3 = -(1+p_var*0.97)*(mu)*(s0+dt*s2/2)*(i0+dt*i2/2)/Npop-Vacc1(ii);
            e3 = (1+p_var*0.97)*(mu)*(s0+(e_var1*v10+e_var2*v20+e_var3*v30)+...
            ...
            v13 = Vacc1(ii) - Vacc2(ii) - (1+p_var*0.97)*(mu)*e_var1*(v10+dt*v12/2)*(i0+dt*i2/2)/Npop;
            v23 = Vacc2(ii) - Vacc3(ii) - (1+p_var*0.97)*(mu)*e_var2*(v20+dt*v22/2)*(i0+dt*i2/2)/Npop;
            v33 = Vacc3(ii) - (1+p_var*0.97)*(mu)*e_var3*(v30+dt*v32/2)*(i0+dt*i2/2)/Npop - omega*(v30+dt*v32/2);
...
%             end
            ...
            s4 = -(1+p_var*0.97)*(mu)*(s0+dt*s3)*(i0+dt*i3)/Npop-Vacc1(ii);
            e4 = (1+p_var*0.97)*(mu)*(s0+(e_var1*v10+e_var2*v20+e_var3*v30)+...
            ...
            d4 = f*gamma*(q0+dt*q3);
            v14 = Vacc1(ii) - Vacc2(ii) - (1+p_var*0.97)*(mu)*e_var1*(v10+dt*v13)*(i0+dt*i3)/Npop;
            v24 = Vacc2(ii) - Vacc3(ii) - (1+p_var*0.97)*(mu)*e_var2*(v20+dt*v23)*(i0+dt*i3)/Npop;
            v34 = Vacc3(ii) - (1+p_var*0.97)*(mu)*e_var3*(v30+dt*v33)*(i0+dt*i3)/Npop - omega*(v30+dt*v33);
            p4 = omega*(v30+dt*v33);
